  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isSignUp) {
        ...
        await setDoc(doc(db, "superAdmin", userCredential.user.uid), {
          email: email,
      ...
      const userDoc = await getDoc(doc(db, "superAdmin", userCredential.user.uid))
      if (!userDoc.exists()) {
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#1B2A4E] to-[#19A5A2] flex flex-col justify-center items-center p-4">
      <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8">
        ...
        <h2 className="text-3xl font-bold text-center text-gray-800 mb-8">
          Super Admin {isSignUp ? "Sign Up" : "Login"}
        </h2>
